...
> **Status:** **Production-Ready Prototype Complete.** Full backend simulation and
> predictive engine (`src/sim_plant/`, `src/shadowline/`), 12-screen React/TypeScript
> dashboard (`Frontend/`), complete business proposal (`BUSINESS_PROPOSAL.md`),
> and 100% passing test suite. See [ARCHITECTURE.md](ARCHITECTURE.md) and
> [BUSINESS_PROPOSAL.md](BUSINESS_PROPOSAL.md) for full design and business case.
...
### 1. Run Backend Services (API & Simulator)
 
```bash
# Install backend in editable mode
pip install -e .

# Run the predictive digital twin API (http://localhost:8000)
python scripts/run_api.py

# (Optional) Run sim_plant factory simulator in parallel
python scripts/run_sim_plant.py

# (Optional) Seed 3000+ historical events & scored predictions
...
```
Frontend/     12-screen React + TypeScript + Tailwind + Recharts dashboard
...
src/shadowline/   the digital twin engine and FastAPI backend (Service B)
tests/        unit, integration, and shared fixtures
BUSINESS_PROPOSAL.md   Full Round 2 Business Proposal & ROI Model
